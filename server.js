...
async function checkIfCollectionExists() {
    let collectionExit = await GameData.findOne({ gameID: "7up7down" });
...
const StartRound = async () => {
    let date = new Date();
    round_id = "ud" + date.getDate().toString() + (date.getMonth() + 1).toString() + date.getFullYear().toString() + "-" + date.getHours().toString() + date.getMinutes().toString() + date.getSeconds().toString();
    ...
    await GameData.updateOne({ gameID: "7up7down" },
        {
            $set: {
